...
Client.prototype.getX509Certificate = function (key, opts, cb) {
	...
	var resps = ['failure', 'x509-response'];
	...
	this.doRequest(frame, resps, timeout, function (err, resp) {
		...
		var certs = [];
		var cs = resp.certificates;
		for (var i = 0; i < cs.length; ++i) {
			try {
				var cert = sshpk.parseCertificate(cs[i],
				    'x509');
				certs.push(cert);
			} catch (e) {
				var err2 = new AgentProtocolError(resp,
