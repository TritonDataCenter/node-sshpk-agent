...
Client.prototype.doRequest = function (sendFrame, respTypes, timeout, cb) {
	...
	req.on('send', function (s) {
		assert.strictEqual(req.timeoutId, undefined);
	...
	req.on('error', function (err) {
		...
		if (req.retries > 0) {
			...
			self.connect(function () {
				req.emit('send', self.encodeStream);
			});
			return;
		}
...
Client.prototype.finishRequest = function () {
	...
	if (this.requestQueue.length > 0)
		this.nextRequest();
	else if (this.socket && this.socket.unref)
		this.socket.unref();
	else if (this.socket)
		/*
		 * If we're on node 0.8 we don't have unref(), so we just have
		 * to disconnect here. :(
		 */
		this.disconnect();
};
...
Client.prototype.connect = function (cb) {
	...
	sock.once('connect', function () {
		...
		assert.strictEqual(self.state, 'connecting');
	...
	sock.once('close', function () {
		if (self.state === 'connected' && sock === self.socket) {
			self.disconnect();
			if (self.request) {
				var e = new Error('Unexpectedly lost ' +
				    'connection to agent');
				self.request.emit('error', e);
			}
		}
	});
