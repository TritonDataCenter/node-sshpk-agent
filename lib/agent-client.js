...
Object.defineProperties(AgentClient.prototype, {
...
AgentClient.prototype.addKey = function (key, opts, cb) {
	assert.object(key, 'key');
...
AgentClient.prototype.removeKey = function (key, opts, cb) {
	assert.object(key, 'key');
	assert.ok(key instanceof sshpk.Key, 'key must be an sshpk.Key');
	if (typeof (opts) === 'function' && cb === undefined) {
		cb = opts;
...
AgentClient.prototype.removeAllKeys = function (opts, cb) {
	if (typeof (opts) === 'function' && cb === undefined) {
		cb = opts;
		opts = {};
	}
...
AgentClient.prototype.enqueueRequest = function (req) {
	assert.object(req);
...
AgentClient.prototype.nextRequest = function () {
