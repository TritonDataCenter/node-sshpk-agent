...
AgentEncodeStream.prototype._transform = function (obj, enc, cb) {
	...
	if (def === undefined) {
		var err = new AgentProtocolError(obj, 'unknown frame type: ' +
		    obj.type);
		this.emit('error', err);
		return;
	}
	...
	for (var i = 0; i < def.args.length; ++i) {
		...
		var v = obj[argdef.name];
		if (v === undefined) {
			var err = new AgentProtocolError(obj, 'missing ' +
		...
		try {
			...
			var err = new AgentProtocolError(obj, 'argument ' +
	...
	if (offset !== frame.length) {
		var err = new AgentProtocolError(obj, 'arguments to ' +
...
AgentDecodeStream.prototype._transform = function (chunk, enc, cb) {
	...
	while (this.frame.length > 4) {
		...
		if (this.frame.length < (len + 4)) {
			...
			if (def === undefined) {
				var err = new AgentProtocolError(frame,
