...
var Certificates = {
	encodeSize: function (v) {
		assert.arrayOfObject(v);
		var len = 4;
		v.forEach(function (cert) {
			len += LenPrefixBuf.encodeSize(cert);
		});
		return (len);
	},
	encode: function (v, buf, offset) {
		...
		offset += 4;
		for (var i = 0; i < v.length; ++i)
			offset = LenPrefixBuf.encode(v[i], buf, offset);
		return (offset);
	},
	decodeSize: function (buf, offset) {
		var start = offset;
		...
		offset += 4;
		for (var i = 0; i < count; ++i)
			offset += LenPrefixBuf.decodeSize(buf, offset);
		return (offset - start);
	},
	decode: function (buf, offset) {
		var v = [];
